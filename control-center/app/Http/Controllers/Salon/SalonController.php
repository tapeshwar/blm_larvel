...
{ 
    ...
    public function __construct() {
        $this->append_js = ['common.js','salon.js'];
    ...
    public function index(Request $request){
        ...
        if(!empty($request->get('q'))){
            ...
            $where['salon_name'] = $heading;
            //$where['title'] = $heading;
        }
        $pages = DB::table('salons')->where($where)->orderBy('salon_id', 'desc')->paginate(10);
        //dd(DB::getQueryLog());
        //die;
    ...
    public function salon_working_time(Request $request,$id=NULL){

        if(!empty($id)){
            $where['salon_id'] = $id;
            $salon_data = DB::table('salons')->select(['salon_name'])->where($where)->first();

            $data = [
                'title' => 'Manage Salon Working Time',
                'heading' => 'Manage Salon Working Time',
                'salon' => $salon_data
            ];
            return view('salon/working_time_modal',$data);
        }
    }
